...
/atom/movable/lighting_overlay/proc/update_overlay()
	...
	if (!istype(T)) // Erm...
		if (loc)
			warning("A lighting overlay realised its loc was NOT a turf (actual loc: [loc], [loc.type]) in update_overlay() and got qdel!")

		else
			warning("A lighting overlay realised it was in nullspace in update_overlay() and got qdel!")
